...
//FIXME: these need to go into platform.cpp
        ...
        FILEFINDBUF3 entry;
        HDIR file_handle = HDIR_CREATE;
        if (DosFindFirst( directory_name, &file_handle,
            FILE_DIRECTORY | FILE_ARCHIVED | FILE_READONLY,
            &entry, sizeof( entry ), &num_entries, FIL_STANDARD ) == 0)
        {
            do
            {
                num_entries = 1;
                int length = strlen(entry.achName);
                ...
                //
                bool is_java  = (length > FileSymbol::java_suffix_length &&
                                 FileSymbol::IsJavaSuffix(&entry.achName[length - FileSymbol::java_suffix_length])),
                     is_class = (length > FileSymbol::class_suffix_length &&
                                 FileSymbol::IsClassSuffix(&entry.achName[length - FileSymbol::class_suffix_length]));

                if (is_java ||
                    is_class ||
                    ((entry.attrFile & FILE_DIRECTORY) && Case::Index(entry.achName, U_DOT) < 0))
                {
                    char *clean_name = new char[length + 1];
                    for (int i = 0; i < length; i++)
                        clean_name[i] = (entry.achName[i] == U_BACKSLASH ? U_SLASH : entry.achName[i]);
                    if (is_java)
                         strcpy(&clean_name[length - FileSymbol::java_suffix_length], FileSymbol::java_suffix);
                    else if (is_class)
                         strcpy(&clean_name[length - FileSymbol::class_suffix_length], FileSymbol::class_suffix);
            ...
            } while (DosFindNext( file_handle, &entry, sizeof( entry ),
                &num_entries ) == 0);
            DosFindClose(file_handle);
        }
...
void FileSymbol::SetFileName()
{
    ...
    this -> file_name_length = (dot_directory ? 0 : directory_name_length) +
                               ...
                               (IsJava() ? java_suffix_length : class_suffix_length);
    ...
    strcat(file_name, IsJava() ? FileSymbol::java_suffix : FileSymbol::class_suffix);
    if (path_symbol -> IsZip())
