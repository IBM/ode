        ...
        for (char *path = option.classpath, *path_tail = &path[strlen(path)]; path < path_tail; path++)
        {
            ...
            *path = U_NULL; // If a seperator was encountered, replace it by \0 to terminate the string.
            ...
            if (chdir( head ) == 0)
            {
                ...
                full_directory_name = new char[directory_length];
                _getcwd( full_directory_name, directory_length );
                int length = strlen( full_directory_name );
#endif
...
FileSymbol *Control::GetFileBoth(Control &control, PackageSymbol *package, NameSymbol *name_symbol)
{
    ...
    //
    // If we are generating dependency files, we must use the .java file.
    //
    if (control.option.makefile && java_file_symbol)
