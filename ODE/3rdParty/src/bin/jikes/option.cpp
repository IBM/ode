...
#else /* OS/2 */
        int directory_length = 1024;
        char *disk_directory = new char[directory_length];

        {
            _getcwd( disk_directory, directory_length );
            int length = strlen( disk_directory );
#endif
                                              ...
                                              dependence_report_name(NULL)
{
    ...
    char *main_current_directory = new char[max_path_len];
    _getcwd( main_current_directory, max_path_len );
    int length = strlen( main_current_directory );
#endif
    {
...
#elif defined(WIN32_FILE_SYSTEM) || defined(OS2_FILE_SYSTEM)
...
#else /* OS/2 */
                if (disk != 0)
                  _chdrive( toupper(disk) - 'A' + 1 );
                chdir( arguments.argv[i] );
                int max_path_len = 1024;
                directory = new char[max_path_len];
                _getcwd( directory, max_path_len );
#endif
