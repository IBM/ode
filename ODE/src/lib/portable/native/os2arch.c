...
#include "lib/portable/native/os2_ar.h"
...
#define STRING_TABLE_NAME "ARFILENAMES/"
#define STRING_TABLE_NAME_LEN 12
...
int ODEreadArchCurrent( ODEArchInfo *buf )
{
  ...
  while (!done) /* some members are special, and are silently skipped */
  {
    ...
    else if (strncmp( arhdr.ar_name, STRING_TABLE_NAME,
        STRING_TABLE_NAME_LEN ) == 0)
    {
      if (buf->longnames != 0)
        free( buf->longnames );
      ...
      continue; /* keep looking for a non-special member */
        ...
        isdigit( arhdr.ar_name[1] )) /* member with a long name */
    {
