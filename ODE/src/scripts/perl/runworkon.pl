...
## wrapper for Workontest ##
use strict;
use Workontest;
OdeEnv::setPlBase();

OdeInterface::logPrint( "\n\tRunning workon Test Script...\n\n" );

# execute workon test
my $status = Workontest::run();
if ($status) 
{ 
  OdeInterface::logPrint("workon test failed!\n");
}
else
{
  OdeInterface::logPrint("workon test passed!\n");
}
