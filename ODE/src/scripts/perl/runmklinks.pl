...
## wrapper for Mklinkstest ##
use strict;
use Mklinkstest;
OdeEnv::setPlBase();

OdeInterface::logPrint( "\n\tRunning mklinks Test Script...\n\n" );

# execute mklinks test
my $status = Mklinkstest::run();
if ($status) 
{ 
  OdeInterface::logPrint("mklinks test failed!\n");
}
else
{
  OdeInterface::logPrint("mklinks test passed!\n");
}
