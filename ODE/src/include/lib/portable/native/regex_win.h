...
#include <stddef.h>
/* the following replaces gregex.h for people who don't have
   legal approval to view 3rd Party source code.  This is just
	so other files have properly sized definitions to use. */

typedef struct
{
  char ptr[16]; /* current size of regex_t in gregex.h */
} regex_t ;

/* mimic the values from gregex.h */

#define REG_NOMATCH  1 /*  1 RE pattern not found         */
#define REG_BADPAT   2 /*  2 Invalid Regular Expression   */
#define REG_ECOLLATE 3 /*  3 Invalid collating element    */
#define REG_ECTYPE   4 /*  4 Invalid character class      */
#define REG_EESCAPE  5 /*  5 Last character is \          */
#define REG_ESUBREG  6 /*  6 Invalid number in \digit     */
#define REG_EBRACK   7 /*  7 [] imbalance                 */
#define REG_EPAREN   8 /*  8 \( \) or () imbalance        */
#define REG_EBRACE   9 /*  9 \{ \} or { } imbalance       */
#define REG_BADBR    10 /* 10 Invalid \{ \} range exp      */
#define REG_ERANGE   11 /* 11 Invalid range exp endpoint   */
#define REG_ESPACE   12 /* 12 Out of memory                */
#define REG_BADRPT   13 /* 13 ?*+ not preceded by valid RE */
#define REG_ECHAR    14 /* 14 invalid multibyte character  */
#define REG_EBOL     15 /* 15 ª anchor and not BOL         */
#define REG_EEOL     16 /* 16 $ anchor and not EOL         */
